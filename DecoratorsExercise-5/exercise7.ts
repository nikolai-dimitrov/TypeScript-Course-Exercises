function addCreatedOnProperty<T extends { new (...args: any[]): {} }>(
	constructor: T
) {
...
function filterCreatedEntitiesForCertainPeriod(timePeriod: number) {
	return function (
		target: any,
		methodName: string,
		...
		descriptor.value = function () {
			...
			const entities = originalMethod?.call(this);
			const newestEntities = entities.filter(
				(entity: User | Employee) =>
					timeNow.getTime() - (entity as any).createdOn.getTime() <
					timePeriod
			);
			if (methodName === "getEmployees") {
				console.log("Method getEmployees called successfull");
			}
...
class UsersService {
	...
	@filterCreatedEntitiesForCertainPeriod(5000)
	@censorData(userCensorService)
	getUsers() {
		return this._users;
	}

	@filterCreatedEntitiesForCertainPeriod(10000)
